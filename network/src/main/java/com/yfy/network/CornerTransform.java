...
public class CornerTransform implements Transformation<Bitmap> {
   ...
   * @param rightTop    rt
   * @param leftBottom  lb
   * @param rightBottom rb
   */
  public void setExceptCorner(boolean leftTop, boolean rightTop, boolean leftBottom,
      boolean rightBottom) {
  ...
  public Resource<Bitmap> transform(@NonNull Context context, @NonNull Resource<Bitmap> resource,
    ...
    if (exceptRightBotoom) {//右下角不为圆角
      canvas.drawRect(canvas.getWidth() - radius, canvas.getHeight() - radius, canvas.getWidth(),
