...
public final class BarUtil {
    ...
    public static int getStatusBarHeight() {
        ...
        @SuppressLint("DiscouragedApi") int resourceId = resources.getIdentifier("status_bar_height", "dimen", "android");
    ...
    public static void setStatusBarColor(@NonNull final View fakeStatusBar,
                                         @ColorInt final int color) {
        if (Build.VERSION.SDK_INT < Build.VERSION_CODES.KITKAT) return;
        Activity activity = ActivityUtil.getActivityByContext(fakeStatusBar.getContext());
    ...
    public static boolean isNavBarVisible(@NonNull final Window window) {
        ...
        if (isVisible) {
            ...
            if (RomUtils.isSamsung()
                ...
                try {
                    return Settings.Global.getInt(Util.getApp().getContentResolver(), "navigationbar_hide_bar_enabled") == 0;
    ...
    private static String getResNameById(int id) {
        try {
            return Util.getApp().getResources().getResourceEntryName(id);
        } catch (Exception ignore) {
            return "";
        }
    }
